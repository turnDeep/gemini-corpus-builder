...
変換時は以下のグローバルリソースを参照して整合性を保持：

1. **グローバル辞書** (`consistency_work/global_dictionary.json`)
   - 全ファイル共通の用語マッピング
   - 固有名詞の統一表記
   - 文体ルールの一覧

2. **文書グラフ** (`consistency_work/document_graph.json`)
   - 文書間の参照関係
   - 時系列的な依存関係
   - トピックの継続性

3. **クラスタ情報** (`consistency_work/clusters.json`)
   - 文書のグループ分け
   - クラスタ別の変換ルール
   - 特徴的な語彙リスト
